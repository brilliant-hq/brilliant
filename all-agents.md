...
- Reads report COMMITTED geometry. While the user is mid-gesture (a pointer drag, a resize, a keyboard nudge), the change is a preview that has not landed in the model yet, so a `get_selection`/`lookup` taken then can return the pre-gesture geometry for any element kind. That is the contract, not a bug: read again once the gesture ends (a nudge burst commits shortly after the last press) before deciding anything from the numbers.

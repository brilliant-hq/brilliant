...
- `mcp__brilliant__objects_result`: after `</objects>`, the result of that block: what you built, the ref ids, notes, and a screenshot (pass your `sessionId`).
...
**DO NOT call `init`, `create_modify_elements`, or `create_html`**: in-app these are refused and nothing is created. Build with `<objects>` tags in your reply instead. They apply live as you write them, with no commit tool needed, and they still apply when the same reply also uses read tools. After `</objects>` call `objects_result` (pass your `sessionId`); anything else you write before that is paused and the result is handed to you. It returns what you built, the ref ids, any lines that did not apply, composition notes, auto-corrections, and a screenshot of your `previewIds`. Read tool calls (`lookup`, `get_selection`, `export`) go before a block or after its result. When a block errors, the halt is retried once with the diagnostic fed back: if the very next block fails with the same diagnostic on the same target, the turn stops and hands it back to the user, so fix the actual failing line rather than re-emitting it.

...
**Tools available by plain names** (no `mcp__` prefix): `get_knowledge`, `get_selection`, `export`, `execute_commands`, `get_blueprint`, `search_elements`, `generate_image`, `plan_agents`, `spawn_agent`.

`#ref` session refs work everywhere — `execute_commands`, `export`, and `get_blueprint` all resolve refs. Refs can be numeric (`#1`) or named (`#card`).
...
After sub-agents finish, **own the result**: inspect the canvas (`get_blueprint` or `export`), catch what they missed, and fix what isn't good enough.

...
## Separator / Divider
Horizontal: `r s(fill,1) f[(#E2E8F0)]`. Vertical: `r s(1,fill)`. With label: H-row `x(c),y(c) g($spacing.3)` — divider + text(12, muted) + divider.
...
## Hero
V-stack `y(c),x(c) g($spacing.6) pad(80-120,$spacing.16) s(fill,hug)`. Optional eyebrow. Title(40-56, sb/b, centered) + subtitle(16-18, muted, `s(500-700,hug)`) + CTA row (H-row `g($spacing.3)`).
...
## Carousel
Clip frame with slides. Dot indicators: H-row `x(c),y(c) g($spacing.2)` — active `c s(8,8)` accent, inactive muted.

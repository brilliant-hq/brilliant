...
al(h,x(c),y(c),g($spacing.none),pad($spacing.xs,$spacing.md)) s(hug,hug) f[($indigo.intense),(f2,glow($indigo.soft,o($visibility.bold),blur(6)))] st[(solid($indigo.soft,o($visibility.soft)),w($stroke.width.subtle))] rd($radius.full) shadow($indigo.soft,y(0),blur(12)) #neon_badge
  t("Live",$font.family,$font.size.xs,sb) f[($indigo.subtle)] #neon_label
```
...
```
al(v,g($spacing.md),pad($spacing.lg)) ds(, theme(dark)) s(340,hug) f[($color.surface)] rd($radius.md) shadow($primary.mid,o($visibility.subtle),y(0),blur(1),sp(1)) shadow($primary.mid,o($visibility.subtle),y(0),blur(16)) #glow_card
  t("Glow Border",$font.family,$font.size.lg,sb) f[($color.text.primary)] "Title" #glow_title
  t("Colored shadow replaces stroke",$font.family,$font.size.sm) s(fill,hug) f[($color.text.secondary)] "Body" #glow_body
```

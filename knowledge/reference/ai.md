...
**Signing in.** Brilliant AI is included on Free every month, and part of it can be used before you sign in, so your first messages send without an account. When that part is used up, your next send opens a **Sign in to keep going, free** prompt that lets you continue (Google or an emailed code), and signing in starts a full month; dismiss it and your prompt is kept in the input so nothing is lost. Subscribing happens inside the editor too, on the web as well as the desktop app: a Personal purchase opens a full-page checkout on the same overlay (you upgrade without leaving the editor), with your account email already filled in, the plan flips live, and the message you were holding continues by itself. The demo (playground), and driving Brilliant from an external tool over MCP, both need no account.

**Free plan limits.** Free gets two things. First, **Brilliant AI** every month (the default lane): included usage that resets monthly with no rollover, shown as a percent used and a reset date in Settings > Usage (type `/usage` in the chat for the same on demand) and on the account usage page; part of it can be used before you sign in, and signing in starts a full month. Second, a cap of **10 messages a day** on the built-in chat on **your own AI**: a message counts the moment the model starts answering, so a send that fails before the first word (a rejected key, say) does not count, and it counts even when your own API key pays for the tokens (this stays true, no chat token routes through Brilliant, so it is a limit on the app feature, not a Brilliant AI allowance). The count resets at the start of the next UTC day, shown in your local time; at the cap the next send opens the upgrade page, which reads "You've used today's 10 messages on your own AI. Personal removes the daily limit." with an Upgrade to Personal button and a note that agents over MCP keep going, unlimited. A message blocked at any wall is held and continues by itself the instant the wall clears (you sign in, upgrade, or add your own AI), while dismissing the page (press Escape, or click outside it) returns the message to the composer as an editable draft, so the message is never lost either way. Upgrading happens without leaving the app, and the held message continues once the plan is active. Any paid plan removes the cap and raises your Brilliant AI usage. Playground replays and external MCP agents never count against either, and external agents stay unlimited on every plan. When the month's Brilliant AI usage is used up, the turn already in flight always finishes first, then the next send opens the upgrade page, a page on the command-palette shell (the same overlay as sign in, with the canvas dimmed behind it) that names when your usage resets and offers the paid rungs and a door to bring your own AI. The one exception: if your usage runs out mid-answer, the answer stops there and the sign-in or upgrade page opens right away, with your message held as usual. Your own AI is never blocked by usage. A tool that was running when usage ran out (an image being generated, say) settles to "Stopped: your Brilliant AI usage ran out". Once the wall clears and your held message continues, that row reads "Paused: allowance used" and one line in the chat says where you stand and that the work continues ("You are on Pro. Picking up where we left off."; "Signed in. Picking up where we left off." after a sign-in; "Using your own AI. Picking up where we left off." after adding your own AI). Reopening the chat later shows the same rows.

**When the provider refuses.** A provider error never shows its raw text in the chat: the card names the provider and the cause in plain words with the one next step, for example "Gemini rejected the request (invalid API key). Update the key in Settings to continue." (with an Update API key button, which opens the key editor of the provider the chat runs on, a custom endpoint's included), "Gemini rejected the request (model not available). Pick another model to continue.", or "Gemini rejected the request (conversation too long). Compact to continue."; a key out of credit reads "Your OpenAI account is out of credit. Add credit at OpenAI, or pick another model."; a provider that stayed busy through the retries reads "Anthropic stayed busy through 3 retries. Send again in a minute."; a connection that drops before the reply starts is retried once by itself ("The connection to Anthropic dropped before the reply started. Retrying."). The provider's own message sits behind the card's Details toggle. On the default Brilliant AI lane, when the upstream model stalls or errors while the proxy relays it, the chat shows an in-place waiting row that says so and keeps retrying, for example "Gemini 3.8 Flash is not responding on Google side right now; Brilliant is fine, retrying (5 s)", and once the wait budget runs out or a breaker opens it settles to a stop card with the same fact and a Send again button. A relayed provider error reads "Gemini 3.8 Flash on Google side: busy (429). Retrying in 5 s." while it retries inside that budget.

**When the request itself goes wrong.** If the model's provider refuses a request because of how Brilliant assembled it (a fault on Brilliant's side, never yours), the chat shows "Something went wrong on our side. Retrying…" and retries the same message once by itself, in the same turn; the answer then continues as if nothing happened. If the retry fails too, the turn ends with "Brilliant AI could not continue this turn. Send your message again.", and sending again starts a fresh attempt. Provider error text is never shown in the chat for these; it goes to the diagnostics log.

**When the chat cannot save to disk.** In the desktop app, each session's transcript is written to disk so it survives a restart. If those writes keep failing (a full disk, a read-only or permission-blocked location, or a synced folder like iCloud or Dropbox holding the file), the chat tells you once, naming the reason in plain words: "Chat history is not being saved to disk (disk full). This conversation stays while the app is open and will not be here after a restart." The conversation still works on screen for now. Freeing space, fixing the folder's permissions, or moving the app's data out of a synced folder lets saving resume. In the web app (browser) the transcript lives in the browser's local storage instead, and when the browser blocks it (private browsing or a full storage quota) the notice is a red tile shown once per session, "Chat history can't be saved in this browser". The conversation still works on screen and is gone after a reload.

**When the AI names a canvas that does not exist.** A design block aimed at a canvas your project does not have never creates that canvas as a side effect. If the project has exactly one canvas, the block simply builds there and the chat says so ("Canvas "PawPatrol" does not exist; building on main instead."). Otherwise the chat shows "Canvas "PawPatrol" does not exist; asking the model to retarget." and the AI is told which canvases exist and retries once on its own; if it names a missing canvas again, the turn stops with a card like "Line 1 of the last design block failed twice the same way, so Gemini 3.8 Flash stopped. The lines before it are on the canvas." (the same card any design block that fails twice the same way gets) and your next message starts fresh.

**When Brilliant stops the model on purpose.** A turn the harness ends (a model looping on elements that are gone, tool calls that do nothing, a reply budget spent on thinking, or all its tool steps used) ends with one card naming the model and what to do, for example "Gemini 3.8 Flash kept editing elements that are no longer on the canvas (Hero card), so the turn was stopped. Send again to continue." or "Gemini 3.8 Flash made 4 tool calls in a row that did nothing, so the turn was stopped. Send again to continue.". A reply that is cut off mid-way continues by itself once (the row reads "Reply cut off; continuing from where it stopped" until it resumes); cut twice, the turn ends with "The model's stream was cut twice; try sending again.". A tool that fails settles its row to the outcome ("Image generation failed: out of credit", or "Image 2 of 3 could not be generated (busy). The other 2 are on the canvas."), with the tool's own message behind Details.
...
- Context-window size varies per model, and the readout reflects the real per-model window. The session shows how much of the context window is LEFT as a bare percentage that starts at 100% and counts down. It rides beside the turn's stopwatch: while the agent is running it sits just left of the elapsed timer and updates live as the answer grows, and once the answer lands it sits just left of the time that answer took, on the last AI message. Hover it for the full breakdown: percent used, tokens used, capacity, and room left until the context is compacted. The same percentage also shows in the composer once less than about half the window is left.
...
1. **Canvas context (a text outline):** the first message of a chat carries a short *text* orientation: the canvas name, the top-level frames with their child counts (no element tree), the design-system token catalog, the component catalog, and the element count at session start. It goes with the first message only, on every provider (later messages add just a one-line note when the user switches canvases, or the new token catalog when the design-system picker changes). The model reads the elements it works on through `lookup` and `get_selection`, and each `<objects>` block's result tells it what changed, including when an element it had read was changed or removed by someone else in the meantime. This is text, not an image; no screenshot of the canvas or screen is captured for this context. Your current *selection* is not included here; send it explicitly with an `@` element attachment or via the `get_selection` tool. To send a prompt with no canvas context, start the chat in an empty workspace.
...
3. **Automatic self-review screenshot (after edits):** after each block of changes, the AI reads that block's result before it does anything else: what landed, notes, and a screenshot of *those changed elements*. It asks for the result itself, or Brilliant hands it over before the AI continues; then it checks its own work (spacing, contrast, alignment, clipping). It is a render of the design content only, never the screen or other apps. Text-only models never receive it, and it is suppressed for the rest of a session once an endpoint rejects image input. After two blocks in a row that change nothing new, Brilliant asks the AI to wrap up with an overview instead of opening another block.
...
- **Batch / parallel:** several images can be requested in one turn. On the free plan, and before you sign in, up to three of them run per turn; the rest are named in the reply, so you can ask for more.
...
- On Brilliant AI, a turn you stop is charged for what was counted or reported before you stopped (the prompt, and the answer generated so far); thinking that was never reported is not charged.

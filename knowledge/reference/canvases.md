...
**Migrating a whole file (plugin path):** the plugin's "Import file to Brilliant" streams every page of a Figma file into a NEW Brilliant project named after the file, one canvas per page, in page order. Each canvas is named after its page with a page-number prefix that keeps the order ("1 Cover", "2 Home"); a file with ten or more pages zero-pads the number to one width ("01 Cover" … "10 Specs") so the pages stay in order even in listings that sort names as plain text. Re-importing matches pages by NAME, prefix ignored, so a project imported before the padding (or after the file's page order changed) converges onto its existing canvases and their prefixes are corrected in place. The project holds exactly the file's pages: no default "main" canvas is left in it (a starter canvas may appear for a moment while the first page arrives, then goes; a `main` canvas you have put anything on is never touched). In the desktop app the new project is **local-only** (a folder under your Brilliant projects directory), whether or not you are signed in: an import lands on your machine and stays yours, and publishing it to the cloud is a separate, deliberate choice you make later. In the browser editor, where there is no local disk, the new project is a cloud project. Empty pages mint no canvas and are skipped; Brilliant's import card counts only the pages that carry content and says so when it finishes ("32 pages imported, 16 empty pages skipped"), so a short tally is never a gap. Two things always come with it:
...
- **A note about components from other Figma files.** Components you pull from a team library import as local copies in the new project (Brilliant never reaches into the library file for you), gathered on the project's "Components" canvas. The report names each library component and how many instances use it, so you can import that library file too and relink, or relink by hand.
...
**Components from a team library get their own canvas.** A component the file uses from a shared Figma library (one that does not live in the file itself) lands on a dedicated "Components" canvas, listed after the page canvases, instead of being scattered next to some page's content; instances on every page reference it from there. When you combine pages onto one canvas, those components land inside a "Components" frame on that canvas instead. Re-importing reuses the same canvas or frame rather than adding another.
...
A cloud-only tab is **live**: it joins the project's realtime session like the browser editor does. Collaborators' edits, cursors, and selections appear as they happen, canvases created, renamed, or deleted elsewhere show up in the explorer, and if the canvas you are on is deleted remotely the tab lands on another canvas with a notice. When the connection drops, editing continues and saves go directly to the cloud; live sync resumes on its own. If durable history briefly falls behind the live session, a small "catching up on history" notice reassures you that your edits are safe and it clears on its own once history catches up (it never shows raw server internals, it belongs to the canvas you are on, and you never have to dismiss it by hand). If the project is deleted or your access ends, the tab says so plainly and stops syncing; close it when you are done reading.
...
A **published** project has a version-history panel on the web: its **History** page (`brilliant.design/{handle}/{project}/history`) lists every checkpoint (named and automatic) and lets the owner restore any of them, whole-project or a single canvas. Edits made on the web are saved to the project continuously, but they add a History entry only when you press Checkpoint (or when the server preserves a state before a risky save); the desktop app also adds automatic checkpoints as you work. A **local, unpublished** folder has no such panel: undo there is per-canvas and within-session only, so use Git or an OS backup (Time Machine) for persistent local history.

...
- A **pin** toggle sits at the top right, beside the search field. It keeps the view open while you place (clicking a card still inserts; the view just stops dismissing) and on clicks away to the canvas. The pin is the one thing that decides dismissal: unpinned, clicking the canvas closes the view whether it floats or is docked; pinned, it stays open everywhere. Hover the pin to reveal the other window options, laid out like the AI chat's header: a toggle between two card sizes, then the dock buttons (full screen, bottom, left, or right, exactly like the AI chat panel; docked panels sit above the bottom toolbar, never covering it). They hide again when you move away. Docking or undocking never resets what you were looking at: an open screen, search, and scroll position carry across.
...
Pressing the rail's "Add library" row opens a small options popover next to it (the same kind of menu as the home screen's "+ New" button, in the Assets view's own look): **Your libraries**, **Browse public libraries**, and **Add by handle**. Esc or clicking away dismisses it. Each option opens its own focused screen with just that one thing on it; to leave, press any rail row, exactly like every other screen here (there is no back button).

The search field at the top of the window stays context-aware: on the component grid it searches components ("Search components…"), and inside a focused add screen it searches that screen's content - "Search your libraries…" filters your list, "Search public libraries…" queries the public catalog. Typing never takes you off the screen you are on, and a query never carries from one screen to another.

- **Your libraries**: projects you can access that are marked as libraries, each with its cover, name, handle, and latest release. Press Add to depend on it at the latest release. The whole list flows down the screen - however many libraries you have, the screen simply scrolls through them all; the top search field filters the list by name or handle.
- **Browse public libraries**: with a row of category pills (All, UI kits, icons, design systems, illustrations, templates, wireframes); the selected pill is filled, the rest are quiet, and the top search field narrows within the picked category. Cards show the cover as of the latest release, the name and handle, a one-line description, a star count, and the latest version. Featured libraries appear first, then the most-starred. The card grid fills the screen and the screen scrolls through it as one page (the pills scroll along at the top); further results load automatically as you near the bottom. Searching or picking a category always starts the page back at its top with the new matches.
- **Add by handle**: just an entry box - the same kind of field the settings use for API keys. Paste a library's handle (`@team/library`) and press Add (or Enter); the library is added at its latest release. If the add is refused, the reason appears in plain words right under the field and clears as soon as you type again.

Adding works **signed out** for public libraries: browsing, adding by handle, and fetching all work without an account (a private handle answers that it was not found or is not public). "Your libraries" naturally needs a signed-in account; without a public catalog connection the Browse option simply is not offered.

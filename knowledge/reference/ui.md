...
- **Tabs**: click one to switch projects, middle-click (or its hover ✕) to close, drag to reorder. Each tab reads `project / canvas` and shows the project's current canvas; a shimmer marks an agent working there right now, and it clears the moment the agent finishes (nothing lingers on the tab afterward). The home button opens the project home screen; the breadcrumb half hides there, so the island becomes just home + tabs.
- **Rename a project from its tab**: double-click the tab **title** (or right-click the tab and choose **Rename**) to rename the project without going back to the home screen. The title turns into an inline text field with the current name selected; type the new name and press **Enter** to confirm, **Escape** to cancel (an empty or unchanged name also cancels, and clicking away confirms). Your typing is kept while the field is open even if you move the pointer off the tab. The renamed tab **stays exactly where it is** in the strip and stays the active tab, and no other project flashes open while it renames. It renames the project exactly as the home screen's "…" → Rename does: a local or synced project renames its folder on disk, a cloud-only project renames on the web (old links redirect). A **view-only** project (one you can open but not edit) offers no Rename, on the tab or in its menu. The tab's right-click menu also has **Close Tab**.
- **A badge on the left of each tab says where the project lives and how public it is:** nothing = local only; a cloud icon = a public cloud project; a sync icon = a project that lives on both your computer and the cloud; a lock is added whenever the project is private. A cloud tab for a project that is **not your account's** (someone else's project you opened, or any cloud project while you are signed out) shows a **dashed cloud** in place of the plain one, so a project you do not own reads differently at a glance; signing in as the owner flips it back to the solid cloud. A folder on your machine whose cloud connection belongs to a different account, or any synced folder while you are signed out, wears the dashed icon too (with the view-only eye when your account cannot edit it); signing in with an account that can edit clears both. While a checkpoint is saving, a sticky "Saving checkpoint…" notification (with a loading spinner) shows the progress; the tab badge stays put.
...
- **Hover the island** to reveal, on its left, a copy button then a checkpoint button (bookmark icon), and (if any notifications exist) a clear-notifications button on the right. For a project with local files the copy button writes the full filesystem path of the current canvas's `.bl` file (or the current asset path) to the clipboard. For a **cloud-only project** (one with no local copy) it copies a shareable link to the current canvas on the web instead, and when you have elements selected that link deep-links the selection, so opening it lands the recipient on this canvas with those same elements selected and framed (the tooltip reads "Copy link to selection"). The tooltip flips to "Copied!" for one second. The left pair and the vim indicator belong to the project on screen, so they are absent on the home screen (and while a switch started from home is still landing); clear-notifications belongs to the app and stays.
- **Checkpoint** (bookmark icon) saves a named version of the project. It appears only where the project you are looking at has a cloud copy to checkpoint: a cloud project you can edit, or a desktop folder you have published. A local-only folder and the scratch sandbox show no checkpoint button, and viewers never see it. Publishing a folder makes the button appear right away. Pressing it raises a small toast with a **name field prefilled with the auto-name** (`Automatic checkpoint` on cloud projects, `Checkpoint #N` on a local desktop repository): type a name and press Enter (or Save) to checkpoint under that name, confirm untouched (or clear the field) for the auto-name, Escape/Cancel to walk away. Below the name is an optional description field ("What changed?", up to 280 characters) for a short note that shows with the checkpoint in the project's History. Also runnable from the command palette as **Create Checkpoint**.
...
- **A paste that takes real time says so.** Paste something heavy (a big Figma selection, a whole design pasted from another project) and a "Pasting…" notification appears with a progress bar that fills as the elements land; it names the stage it is in when there is one ("Fetching images…" while image bytes are downloaded, "Uploading assets…" while a cloud project stores them), and finishes with a brief "Pasted". A paste that lands immediately shows nothing at all, and a paste that fails shows only its own error: never a "Pasted" on top of it. The point is that Cmd+V never looks like it did nothing, so there is no reason to press it twice.
...
- **An update is a CTA notification.** When a new Brilliant version is ready, one notification names it and offers **Update** (it downloads, installs, and relaunches from right where you are) and **What's new** (the changelog). It shows its progress in place, comes back on the next check if you close it, and disappears once you are on the current build. The small toolbar indicator and the Updates palette page still work as before.
- **Paid features knock, they don't block.** Hitting a paid wall (a short handle, making a project private, publishing a private drop, a second guest editor, a team whose Team plan lapsed) opens the same sign-in-like page over the editor (the command-palette shell, canvas dimmed) that states the reason and offers one next action ("See Team plans" or "Create a team" for team features); press Escape or click outside the page to put it away and keep whatever you were doing. Locked paid toggles still show a quiet "Available on Personal" chip, and tapping one opens the same page: the toggle never flips until the plan does. You can also reach the checkout directly by typing **Upgrade** in the command palette (the row shows when you are signed in and the session is editable); it opens the same in-editor checkout the walls do.
...
- **"Some of Brilliant's fonts failed to load"** is a warning the app raises once at startup (Windows and Linux) when the fonts it ships with could not be loaded from its install folder, which is what makes menus and panels render in an unfamiliar system font and toolbar icons render blank. It is not a design problem in your project. The usual causes are an antivirus that quarantined files under the app's `data\flutter_assets` folder, a system font changer or font-rendering hook, or an incomplete install; adding Brilliant to the antivirus exclusions and reinstalling normally fixes it. The notification's button opens the logs folder (`~/.config/brilliant/logs`), where `brilliant.log` carries one `chrome_font_probe` line naming each affected font file and the exact error. When the blocked files are the app's own interface font or toolbar icons, Brilliant repairs itself: it switches to a copy of those fonts and icons built into the app, the interface renders correctly, and the notice reads **"Brilliant restored its fonts from a built-in copy"** instead (an information notice, same button). No reinstall is needed after a repaired start: add an antivirus exclusion for the Brilliant folder so the next start loads the installed fonts directly, and reinstall only if the interface still looks wrong.
...
On the web, a tab left open across app updates keeps itself current: if it can no longer load its own assets because a newer version has superseded it, it reloads once onto the current version (landing you back on the same canvas) instead of degrading into a broken editor.
...
- **Cloud**: files that live in the cloud only, including projects other people have shared with you (a role granted on their project). A shared project wears the dashed cloud icon, plus the view-only eye when your role is viewer, and opens like any other cloud project.
...
- **Remove from list**: drops the project from your list. It never deletes anything on disk or in the cloud. This works on the **Scratch** sandbox too: home normally always offers a Scratch row, and removing it puts that row away for good. Your scratch work is untouched on disk, and the row comes back if Brilliant ever opens the scratch workspace again (which it does when you have no other project to open). Removing a row only changes what your home screen lists: the project still exists, and an agent can still address it by name (it stays in `list_projects`).

While searching with no matches the grid shows "No projects match." With nothing to show at all, it reads "No projects yet" and the left panel's "+ New" is the way on.
...
| Toggle desktop icons (macOS-only; overlay mode only) | Ctrl+I |
...
**Window size and position persist across launches on macOS and Windows.** Relaunching restores the previous studio window frame, and a window left zoomed (macOS: Option+green, or the maximized state on Windows) comes back filling the same display's work area. The frame is saved as you move and resize, and again on quit. Only frames you are responsible for are saved: whatever happens to the window in the first moments of a launch is ignored, so a tiling or auto-arranging window manager (AeroSpace, yabai, Amethyst, Moom and friends) laying the window out at startup does not overwrite the size and position you left behind. Move or resize the window yourself and that frame is saved as usual. A frame saved on a since-disconnected monitor, or on one whose resolution changed, is pulled back fully inside a live display's work area instead of opening off-screen; on macOS that work area already excludes the menu bar (so the title bar never lands under the notch) and the Dock. Fullscreen is deliberately not persisted: leave fullscreen and quit, and you get the last normal frame back. The first run opens centered at about 80% of the primary work area (capped at 1400x900 logical), and the window cannot shrink below 800x600. Overlay mode never affects the saved frame: the studio frame is stored before entering overlay, and the overlay's own fullscreen layer is never written to it.

The overlay-mode toggle is a global hotkey only when the opt-in is on, so it works even when Brilliant is not the focused application. Reassigning the keybinding re-registers the global hotkey. Passthrough is a regular in-app shortcut gated to overlay mode (its WhenClause requires `!isStudioMode`, so the overlay window must be active). Toggle desktop icons is gated to overlay mode as well: it fires only while the overlay window is active, exactly like passthrough. Hiding the desktop restarts Finder for a clean overlay screenshot, so triggering it in a normal studio window (by keyboard or from an AI agent) is refused with a message telling you to enter overlay mode first. It remains a macOS-only operation (the command hard-returns off macOS).
...
Canvas files (`.bl`) and design-system files (`.ds`) display **without** their extension (e.g. `rip figma`, `default`), the way Finder and Figma hide known design-file extensions. Inline rename, though, shows the **full filename** with just the basename selected, so the extension is visible and you can change it on purpose. Changing a canvas's extension is an honest **conversion**, confirmed Finder-style: rename a `.bl` to `.txt` and it becomes a plain text file that opens in the code editor (the prompt lets you keep `.bl` instead); rename a text file to `.bl` and it becomes a canvas, but only if its contents are a valid Blueprint (otherwise the rename is refused and the file is left untouched). A plain rename never changes the extension by accident. Conversions work in synced projects too: the change travels to the cloud as a rename, so collaborators see the file convert rather than vanish. On a cloud-only project tab (no folder on this machine) the conversion is refused with a note pointing you to Make available offline first. Asset files keep their extension everywhere (`hero.png`, `notes.md`). The real filename on disk always carries the extension; paths and ids are unchanged.
...
- A **switch icon** marks a **boolean slot**: the on/off wrapper a Figma boolean property imports as (a tiny two-state component instance around the shown/hidden child). The icon is filled when the slot is on, an outline when off, so you can read the state at a glance instead of seeing what looks like a duplicate layer. Hovering it names it a switch. It behaves like any other layer (click to select, rename, reorder).
...
**Current Stroke / Current Fill Sections**: Default stroke and fill used for newly created elements, plus the creation-style checkboxes (whether a new shape gets a fill / a stroke) when a shape tool is active. Visible only while nothing is selected: the moment there is a selection, its strokes and fills belong to the Selection Strokes / Selection Fills sections instead, so each color affordance has exactly one home in the panel. Press Escape to deselect and the defaults come back.

**Element Section**: X/Y position (the row also holds the absolute-position pin button when the selection is inside an auto layout frame, and the "..." expander), W/H with a constrain-proportions toggle, sizing behavior (hug/fill/fixed per axis) ending in a ruler-icon toggle that reveals Min/Max W/H rows (auto layout frames and their direct children), a pin-constraints row (Figma's nine-nub widget plus horizontal and vertical pin dropdowns, shown for children of frames, components, instances, and non-hug groups and for absolute-positioned auto layout children, hidden where pins are dormant), rotation, opacity, corner radius (expand for per-corner or top/bottom pairs; an app-glyph toggle appears at nonzero radius and reveals a smoothing percent row for squircle corners), flex (for auto layout fill children; moves to its own row when the smoothing toggle shows), and circle arc properties (start angle, sweep, inner ratio) for circle elements. In vector edit mode with nodes or handles selected: shows node/handle position and point type instead of W/H/rotation/opacity. Boolean operations (union, subtract, intersect, exclude, use as mask) show as their own inline row whenever 2+ elements are selected, right in the section without expanding anything. The "..." expander exposes blend mode (containers get a Pass through default plus Normal and the standard modes), absolute scale, alignment, and arrange.
...
**Figma Import Section**: Figma file URL import. Visible only when nothing is selected and the Move tool is active (same condition as Canvas Section). Present in the browser editor too, on projects you can edit; view-only projects never show it. Failures name their cause and fix (expired connection offers reconnect in place with your link preserved, rate limits state the real wait, large files say Figma is still preparing them); big files can take a minute or two on the first fetch and the section says so instead of spinning silently. Before the full download, a quick look at the file catches features URL import would lose (shared-library components, liquid glass, unservable fonts); when it finds any, the section shows a card naming the loss for that file and offers the Figma plugin or "Import anyway". A file with none of those markers, or a check that could not run, imports straight through.

Fonts on Figma import: text keeps its real font whenever Brilliant can serve it on this machine, which covers bundled fonts, the whole Google Fonts catalog (downloaded on demand), fonts installed on the machine, and on macOS the Apple system families Figma names as SF Pro, SF Compact, SF Mono, and New York (they resolve to the installed San Francisco / New York faces automatically). A font Brilliant cannot serve, like a private icon font, imports through the Figma plugin as pixel-true glyph outlines instead of wrong-font text: the element is named "... (outlined: FontName)" and a warning names the font with a count. Outlined text is exact but no longer editable as text; installing the font and re-importing restores editable text. Imports through the file URL cannot outline (Figma's API does not share text outlines), so unavailable fonts there render on a fallback face with the same loud warning.

Glass effects imported through the file URL use calibrated Figma-default parameters (Figma's API strips the real ones), so they look close to Figma out of the box; a design with glass, variables, or library components raises the plugin-upgrade card offering the one-minute in-place upgrade to full fidelity.
...
2. **Quick palette access:** Global Search (Cmd+K), Keyboard Shortcuts (Shift+?), Assets (Cmd+Shift+A; see "Assets View" below).
...
4. **AI input area:** input field, processing/success/failure indicators, collapse chevron, the collapsed-state sparkle chat toggle and its re-expand chevron, running-agent activity bars.
...
A collapse chevron collapses the input. When collapsed, a **sparkle button** replaces the field, with a right-facing chevron beside it: the sparkle opens or closes the AI chat (it stays highlighted while the chat is open), and the chevron re-expands the input field. When the AI chat panel is open, the bottom toolbar shows that same highlighted sparkle (click it to close the chat); while the input is still collapsed a chevron sits beside it that, in one press, closes the chat and brings the inline field back (with the input already expanded there is no chevron: closing the chat via the sparkle returns the field on its own); the chat input lives in the panel above.
...
There is no connection dot on the bar anymore. When the AI input is collapsed, the sparkle toggle described above opens or closes the AI chat (not Settings), and the chevron beside it re-expands the input. Provider status and API keys live only in Settings -> Your own AI (Cmd+, then the Your own AI page), where each provider (Claude Code, Codex, Cursor, Anthropic, OpenAI, Google, OpenRouter chat providers; Quiver for SVG generation) shows whether it is connected. Settings carries **Account**, **Usage** (both lanes: your Brilliant AI percent used and reset date, plus the your-own-AI card) and **Your own AI** (the tab previously called AI Providers). The Usage page updates itself (it refetches when you open it and again after each Brilliant AI message) and carries a **Refresh** button on the Brilliant AI card to pull the latest numbers on demand; on a successful refresh it briefly turns into a green checkmark so you can see it updated, and if a refresh cannot reach the server it leaves one quiet line under the meter ("Could not refresh. Check your connection and try again.") until the next successful refresh.
...
Running a command from the palette normally closes it and gets out of the way. Two exceptions: a command with a checkbox (a setting you might flip twice) keeps the palette open so you can see the checkmark change, and a command whose whole job is to open a palette surface (Settings, Project, Keyboard Shortcuts, Updates, Combos, the color selector, or any of the search scopes) leaves you on the surface it just opened rather than closing it under you.
...
The project-scoped surface (publishing, sharing, cover, library) is the **last tab in Settings, named "Project"** (Cmd+, then Project; it was previously named "Sharing"). A published project heads the pane with its `@handle/project` path, the thing being shared. It only appears when a project context exists (a published project, or an unpublished desktop folder with a signed-in identity). Opened directly from the **Project** command, the account-avatar menu's "Project settings" row, or the copy-link button when the folder isn't published yet. Rows:

- **Publish / Published:** a settings row with a checkmark at the end showing whether the project is live on the web; its title is state-derived, reading "Publish" until the project is live and "Published" once it is. Ticking or un-ticking it is how you publish or unpublish: it never changes anything on the spot, it raises an "are you sure?" confirm on the notification rail that spells out exactly what the change means, and only that confirm's button acts. Unpublishing also asks you to type the project name into the confirm before its button arms. Links stop working immediately; the project is recoverable by support for a grace period before permanent deletion. On desktop, unpublishing leaves your local files untouched, only the cloud copy is removed, so the folder simply returns to unpublished. The checkmark follows the server, so it only clears once the unpublish really landed. Once the project is published, this row also carries two small buttons that anyone can use (not just the owner, since both are read-only): **Copy link** (copies the project's public URL, the icon flips to a check for a moment) and **Open in web** (opens the public page in your browser).
- **Visibility** (project owner only): a toggle between public and private. Flipping it doesn't change anything on the spot: like the Published row, it raises an "are you sure?" confirm on the notification rail first, and only that confirm's button acts. Going private warns that public links stop working immediately (people you've invited keep access); going public warns that anyone with the link can view it and it appears on your public profile. Private is a paid feature: if the account isn't on a paid plan, the server declines and shows a "Paid" badge, and choosing **Upgrade** opens the upgrade flow in place rather than sending you to a pricing page. A Personal purchase completes inside the app (a checkout page on the same overlay, hosting Paddle) and the plan flips live, or you can open the same checkout in your browser from the footer (where your browser's password manager or Apple Pay work), and the app still updates itself when the payment goes through; while that checkout page is open the editor toolbars step aside so the page has the screen to itself, and they return the moment you close it; a switch between paid rungs happens in place on the plan sheet, and that in-place switch is for a live subscription you bought: if your plan came another way (a gift or a comped plan, which has no subscription behind it to change), choosing a rung opens the checkout and buys it, the same door the editor takes, rather than sending you to a settings page with nothing to manage; creating a team finishes in your browser and the app updates itself the moment the plan flips. On desktop the wait stays honest about slow payments: if confirmation takes a while it says "taking longer than expected" and keeps checking on its own, with a **Check again** button to nudge it; and if your browser will not open, it shows the checkout link as copyable text (with a retry) so you can still finish. Flipping to private stops any outstanding share links from working (private means private).
...
A floating window for browsing and placing components, from this project and from every library the project depends on. Open it with **Cmd+Shift+A**, the **Toggle Assets** palette command, or its bottom-toolbar button; Esc or a click away closes it. (While the AI chat is open, Cmd+Shift+A toggles the Chat Explorer instead; use the palette or the button then.) Left rail: "This project", one row per library, and "Add library"; main area: a searchable component-card grid grouped by canvas. Pressing "Add library" opens a small options popover (like the home screen's "+ New" menu, in this window's own look) with three choices - Your libraries, Browse public libraries, Add by handle - each opening its own focused screen; Add by handle is a single entry box like the settings' API-key fields. The search field at the top is context-aware: it searches components on the grid, and inside a focused add screen it searches that screen's content ("Search your libraries…" / "Search public libraries…", in place). Click a card to place it on the canvas, or drag it out of the window. A pin toggle at the top right keeps the window open while placing and on clicks away, in any position (unpinned, a canvas click closes it, docked or floating); hovering the pin reveals the other window options (card size, and docking to the window edges the way the AI chat panel does, always ending above the bottom toolbar), laid out like the AI chat header. The full walkthrough, including adding, updating, and publishing libraries, is in [libraries.md](./libraries.md).
...
**Share links** are a read-only door to a **private** project: anyone with the link can view it without signing in, and can never edit. The design's fonts load for these viewers too, so shared text keeps its real typeface instead of falling back to a default. They live in the project Settings page and are a **paid** feature to create (a public project offers none, since its address is already viewable). Each link is a short `brilliant.design/s/...` address you copy, it never expires, and it keeps working **until you revoke it**. Flipping the project back to public, or revoking a link, cuts off that access right away.

**The playground** (`brilliant.design/playground`, no account needed): a scratch editor with starter prompt pills in the chat. Picking a prompt replays a real agent session building live on the canvas; everything it makes is a real, editable element. Nothing in the playground persists until you keep the design as a project: once you have made something of your own, a toast offers it (`Sign in to keep it` when signed out, `Save as project` when signed in); keeping it turns the canvas you are looking at into your project in place (the address becomes `/{handle}/{project}`, nothing reloads, and edits save from then on; on the free plan the project is public). Signed in, an unsaved scratch is also kept on that device for one hour, so a refresh or a crash brings it back with a `Restored your unsaved scratch` toast offering `Save as project` or `Discard`; signed out, nothing is kept and leaving the page loses the scratch (the browser warns first). The chat transcript itself is not kept.

**The onboarding walkthrough** is one guided step, the same on the desktop app and in the playground. It runs once (per install, or per browser): pick a starter prompt and watch your agent generate real, native elements. The card waits for you to click a prompt; there is no skip button on the card and Esc does not close it, but opening a new chat ("+", Cmd+N, or /new-chat) leaves the onboarding window at any time and lands a real Brilliant AI chat; the walkthrough card ends with the window, so the command palette (Cmd+Shift+P) and Settings open normally again. It is a recording, so clicking a prompt spends nothing. When the build finishes, the canvas glides to center the result. `Reset onboarding` (command palette) re-arms it. Picking that first starter prompt also leaves the demo, so your next message is a real chat on Brilliant AI (or on your own AI if you chose one), and a follow-up in the replayed chat opens a fresh real chat, without an explicit "Exit playground mode".
...
- **Create from Figma**: paste a Figma file link into the inline field and press the button. It creates a project named after the file and imports the whole file (every page becomes a canvas). If your Figma account isn't connected yet, the same button routes through Figma sign-in first and returns you to the menu with the link preserved. Every failure names its cause and its fix (bad link, no access, expired connection, Figma rate limits with the real wait, files too large for the browser); big files can take a minute or two while Figma prepares them and the progress line says so. When the file has more than Figma's API shares (design-system variables, live component wiring), a card offers the one-minute plugin upgrade after the import: open the file in Figma with the Brilliant plugin panel open and the imported project upgrades in place, with the design system landing as a brand and matching component copies becoming live instances. Your edits survive; anything the upgrade cannot re-link is kept as-is and named.
...
- Text in the rendered preview is selectable, and Cmd+C (macOS) / Ctrl+C (Windows) copies the selection. If the editor pane also has a selection, the editor's wins; copying with nothing selected never overwrites what's already on the clipboard. The preview is read-only, so cut does nothing there.
...
The chat panel surfaces AI design sessions above the bottom toolbar. Two lanes through the same UI: **Brilliant AI** (the default, Brilliant-provided models included in every plan, selected by default in a new session) and, below it, your own providers, unchanged: Claude Code (local CLI), Codex (local CLI, ChatGPT subscription), Cursor (local CLI, Cursor account), Anthropic HTTP, OpenAI HTTP (Chat Completions + Responses API), Google Gemini, and OpenRouter. Free includes Brilliant AI every month (usage that resets monthly, shown as a percent used with a reset date in Settings > Usage, and part of it can be used before you sign in); a key you bring never routes through Brilliant. On the Free plan the built-in chat on your own key is capped at 10 messages a day (it counts messages you send); at the cap the next send opens the upgrade page (Upgrade to Personal removes the daily limit, and agents over MCP keep going, unlimited); external agents over MCP are unaffected and stay unlimited. When the month's Brilliant AI usage is used up the next send opens the upgrade page, a page on the command-palette shell (the same overlay as sign in, with the canvas dimmed behind it) that names when your usage resets and offers the paid rungs and a door to bring your own AI; the turn in flight always finishes, except when your usage runs out mid-answer, where the answer stops there and the page opens right away (your message is held either way); your own AI is never blocked by usage. See `ai.md` for providers, models, slash commands, attachments, the usage meter, the daily cap, and tool execution.
...
- **Input bar:** attach button (image, file, screenshot, canvas reference), model selector (per session), thinking-level selector (provider-dependent; its readout reads **Thinking: Off / Low / Medium / High**, defaulting to **Thinking: Off** on Brilliant AI), context-usage indicator, send / stop button. There is no in-panel undo/redo: undo/redo applies to the active canvas via Cmd+Z / Cmd+Shift+Z.
...
**Right-click on element** (in order): Select This Item + Add to Selection (if not already selected), Cut, Copy, Copy as (PNG, PNG @2x, PNG @4x, WebP, SVG, HTML, React, CSS, Blueprint), Copy element id, Send to (Figma; enabled only when the Figma plugin is paired), Paste, Duplicate, Delete, Rename, Group/Ungroup/Add Auto Layout + Operations submenu (Union, Subtract, Intersect, Exclude, Mask) when 2+ elements, Component submenu (Create Component, Create Instance, Detach Instance, Go to Master, Reset Overrides, Push Overrides to Master) when applicable, Arrange (Bring to Front, Bring Forward, Send Backward, Send to Back), Align & Transform (for multi-selection: Align Left/Right/Top/Bottom, Align Horizontally/Vertically, Distribute Horizontally/Vertically; always: Flip Horizontally/Vertically, Scale Up/Down, Fit to Parent), Select Parent (for nested elements), Toggle Clip Content (for frames), Export as (PNG, PNG @2x, PNG @4x, SVG, Replay), Text submenu (for text elements: Bold, Italic, Underline, Align Text Left/Center/Right, Size Auto Size/Auto Height/Fixed Size, Switch Font), View (Zoom to Elements, Center on Elements).

**Copy element id** (next to Copy link, on every platform; also a keybindable command "Copy element id" in the command palette) puts the selected element's id on the clipboard as plain text, one id per line for a multi-selection, and confirms with a "Copied element id" toast. Paste that id into a Claude Code prompt to point the agent at exactly that element: it is the same id `lookup` returns and `create_modify_elements` targets, so the agent can act on the frame by id instead of by description.

...
A floating window for browsing and placing components, from this project and from every library the project depends on. Open it with **Cmd+Shift+A**, the **Toggle Assets** palette command, or its bottom-toolbar button; Esc or a click away closes it. (While the AI chat is open, Cmd+Shift+A toggles the Chat Explorer instead; use the palette or the button then.) Left rail: "This project", one row per library, and "Add library"; main area: a searchable component-card grid grouped by canvas. The search field at the top is context-aware: it searches components on the grid, and while the add-library screen is open the same field searches libraries instead (your own and the public Browse listing, in place). Click a card to place it on the canvas, or drag it out of the window. A pin toggle at the top right keeps the window open while placing; hovering the pin reveals the other window options (card size, and docking to the window edges the way the AI chat panel does), laid out like the AI chat header. The full walkthrough, including adding, updating, and publishing libraries, is in [libraries.md](./libraries.md).

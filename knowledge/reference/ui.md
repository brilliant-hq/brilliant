...
The chat panel surfaces AI design sessions above the bottom toolbar. Two lanes through the same UI: **Brilliant AI** (the default, Brilliant-provided models included in every plan, selected by default in a new session) and, below it, your own providers, unchanged: Claude Code (local CLI), Codex (local CLI, ChatGPT subscription), Cursor (local CLI, Cursor account), Anthropic HTTP, OpenAI HTTP (Chat Completions + Responses API), Google Gemini, and OpenRouter. Free includes Brilliant AI every month (usage that resets monthly, shown as a percent used with a reset date in Settings > Usage, and part of it can be used before you sign in), and a key you bring never routes through Brilliant. On the Free plan the built-in chat on your own key is capped at 10 messages a day (a message counts once the model starts answering), and at the cap the next send opens the upgrade page (Upgrade to Personal removes the daily limit, and agents over MCP keep going, unlimited), while external agents over MCP are unaffected and stay unlimited. The usage wall itself (the upgrade page it opens, the held message, the in-editor checkout) is ai.md's. See `ai.md` for providers, models, slash commands, attachments, the usage meter, the daily cap, and tool execution.

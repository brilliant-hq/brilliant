...
**The onboarding walkthrough** is one guided step, the same on the desktop app and in the playground. It runs once (per install, or per browser): pick a starter prompt and watch your agent generate real, native elements. The card waits for you to click a prompt; there is no skip button and Esc does not close it. It is a recording, so clicking it spends nothing. When the build finishes, the canvas glides to center the result. `Reset onboarding` (command palette) re-arms it. Picking that first starter prompt also leaves the demo, so your next message is a real chat on Brilliant AI (or on your own AI if you chose one), and a follow-up in the replayed chat opens a fresh real chat, without an explicit "Exit playground mode".

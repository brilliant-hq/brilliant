...
Drag any of the 4 image rotation handles (near corner midpoints) to rotate the image within the container.

### Resize Container

Drag the blue L-bracket handles at the element's corners or edge midpoints to resize the element boundary. The image position adjusts automatically (see Crop Compensation below).
...
When you resize or rotate an element **outside crop mode** while holding the **Command key**, the crop data adjusts automatically so the image's world-space position is preserved (the image stays fixed while the container changes around it).

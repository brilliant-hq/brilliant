...
Lower numbers = lighter. Higher numbers = darker. The seed alias (e.g., `brand`) maps to the original seed color value. The `.50` step is generated from the OKLCH scale at 50% lightness, which is close but not identical to the seed.

**Dark mode inversion:** In dark mode, color steps are mirrored — `.5` (lightest) uses the light mode `.90` value, `.10` uses `.80`, etc. This ensures light backgrounds become dark and vice versa while maintaining the same semantic usage of token names.
...
Create a `.styles` file in your repository root. The format is valid YAML — hex colors must be quoted:
...
Typography tokens set font size, weight, family, and line height in one operation. Manually changing any individual property (e.g., font size) clears the typography token link.

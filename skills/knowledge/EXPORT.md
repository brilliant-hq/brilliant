...
description: "Export and import in Brilliant: PNG, JPEG, SVG, PDF, MP4, MOV export, Copy As clipboard commands, and SVG/image import."
---
...
Brilliant supports exporting designs to multiple formats and copying to clipboard in various representations. It also supports importing SVG files and images.
...
| Export to MP4 | Command palette | Export selected elements as MP4 video |
| Export to MOV | Command palette | Export selected elements as MOV video |
...
- **CSS**: CSS class with `width`, `height`, `position`, `left`, `top`, `border-radius`, `background-color`, `border`, `transform` (rotation), gradients, and text properties (`font-size`, `font-family`, `font-weight`, `font-style`, `text-decoration`, `line-height`, `color`).
- **YAML**: Full element properties including hierarchy. Useful for debugging or programmatic access.
- **Blueprint**: Brilliant's internal element DSL format. Can be pasted back into Brilliant or shared with AI tools.

---

## Video Export (MP4 / MOV)

Video export renders animated shader fills frame-by-frame into a hardware-accelerated video file (macOS).

### Workflow

1. **Select elements** on the canvas
2. In the **Export panel** (right toolbar), choose **MP4** or **MOV** format
...
| **.sketch** (Sketch) | No | **Yes** | Brilliant can export to `.sketch` but cannot import from it. To migrate from Sketch, export as SVG and import |
| **.fig** (Figma) | Via Figma URL (API) | No | Paste a Figma URL → "Import from Figma" in the command palette (OAuth). Does not import `.fig` files directly |
...
| **Import from Figma** | Command palette "Import from Figma" (OAuth-authenticated Figma API import) |
...
**Cmd+V** handles three clipboard types:
